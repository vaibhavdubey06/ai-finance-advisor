...
const Dashboard = () => {
  ...
  // Calculate total savings from monthlyData for instant UI feedback
  const totalMonthlySavings = monthlyData && monthlyData.length > 0
    ? monthlyData.reduce((sum, m) => sum + Number(m.savings), 0)
    : 0;

  // Use totalMonthlySavings for progress calculation if goals exist
  const goalObj = goals && goals.length > 0 ? goals[0] : null;
  const progress = goalObj && goalObj.amount > 0
    ? clamp((totalMonthlySavings / goalObj.amount) * 100, 0, 100)
    : 0;
              ...
              <div style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.08)', padding: 32, maxWidth: 420, width: '100%', display: 'flex', flexDirection: 'column', gap: 24, margin: '0 auto 32px auto', color: '#222' }}>
                ...
                <div className="mt-8 flex flex-col items-center">
                  {goalsLoading ? (
                    ...
                    <GoalProgressCard goal={{ ...goalObj, savings: totalMonthlySavings }} progress={progress} />
                  )}
                </div>
              </div>
            </div>
